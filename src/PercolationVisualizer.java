...
public class PercolationVisualizer {
	...
	public final int DEFAULT_DELAY = 1000 / 20; // in milliseconds

	
	private int mySize;
	private IPercolate myPerc;
	public PercolationVisualizer(int n, IPercolate p) {
		mySize = n;
		myPerc = p;
		initDisplay();
	}
	...
	// ask percolator what sites are open, blocked, or full and
	// draw accordingly 
	private void drawGrid(){
		// check if full, open, or blocked
		for (int row=0; row<mySize; row++)
			for (int col=0; col<mySize; col++){
				if (myPerc.isFull(row, col))
					draw(row, col, mySize, Color.CYAN);
